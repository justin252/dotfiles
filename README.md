...
- Symlinks `~/GEMINI.md` to `.agents/AGENTS.md` (Gemini global fallback context)
...
tools/review-output          # Create/update output.md and run Codex/Gemini review
...
tools/sesh                   # Session notes browser: ~/.claude/sessions/ and ~/.codex/sessions/
tools/refresh-skills         # Rebuild shared skills, sync Claude/Gemini, copy Cursor skills
...
- `~/GEMINI.md` – symlinked to shared `AGENTS.md` for Gemini global fallback
...
- `refresh-skills` – rebuild `~/.agents/skills` from dotfiles sources, then re-sync Claude skill symlinks, re-copy Cursor skills, and generate Gemini slash commands (run after editing skills, or via `pull-dot`)
...
- `review-output` – create/update `output.md` and run Codex/Gemini review into `review.md` for the current topic. Respects `$AGENT_REVIEWER`. Use `--background` for checkpoint-style async review
...
- `sesh` – session notes browser: `~/.claude/sessions/` and `~/.codex/sessions/`. Condensed .md summaries for humans; pass context to new sessions. `sesh <query>` pre-filters.
...
Personal global prefs (synced via this repo):
  ...
  ├── Gemini CLI             @import or read via ~/GEMINI.md (native)
