...
shell/tmux.conf              # tmux config (C-a prefix, vim nav, agent status bar, C-a a popup)
...
- `ag` – agent session manager: one command for parallel agent work. `ag <name>` auto-creates worktree + launches agent (defaults to Claude; use `--gemini` or `--codex` to switch). `ag <name> -m MSG` with initial task. `ag` fzf dashboard across all repos. `ag status` cross-repo unified view. `ag kill <name>` kill session. `ag clean` sweeps dead sessions + merged worktrees (`--force` skips prompt). Name = branch = status bar label everywhere.
