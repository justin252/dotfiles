...
- `wt` – navigate to any branch or PR. `wt <branch>` finds existing worktree or creates one. `wt 90` or `wt <PR-URL>` resolves PR to branch. `wt` fzf switch. `wt -d` delete. `wt list` show all. `wt clean` cleanup.
